import { FC } from "react";

const Footer: FC = () => (
  <footer className="w-full flex flex-wrap items-center justify-center gap-x-4 gap-y-1 py-3 px-6 text-xs text-subtext-0 border-t border-mantle">
    <span className="flex items-center gap-1 text-nowrap">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 1024 1024"
        className="size-8 fill-current"
      >
        <path d="M689.414 457.075l57.233-.196.837 243.787-57.233.197zM306.561 329.641h57.235v242.948h-57.235zm127.337-.775l57.235-.13.838 371.125-57.235.13zm-255.515-.064h57.237v371.963h-57.237z" />
        <path d="M512.005 0C228.872 0-1.475 229.68-1.48 511.996c-.002 282.316 230.343 512 513.474 512.004 283.133 0 513.482-229.68 513.484-511.996v-.008C1025.477 229.684 795.134.004 512.005 0zM963.56 512v.004c-.002 248.172-202.573 450.075-451.563 450.075-248.992-.004-451.559-201.91-451.555-450.083.003-248.172 202.573-450.075 451.563-450.075 248.986.004 451.553 201.907 451.555 450.075z" />
        <path d="M678.541 396.794l.233-57.234 206.924.84-.232 57.233zm-116.467 60.183h57.235v242.949h-57.235zm-.834-127.872l57.231-.602.837 79.582-57.232.602z" />
      </svg>
      MIT Licensed 2026
    </span>
    <span aria-hidden="true">·</span>
    <span>
      Hecho por <span className="text-text">Maldo Profe</span>
    </span>
    <span aria-hidden="true">·</span>
    <span>Bs. As. | Arg.</span>
  </footer>
);

export default Footer;
